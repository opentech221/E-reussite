...
const Chatbot = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const initChatbot = async () => {
      try {
        const simpleChatbot = new SimpleEducationalChatbot(user?.id);
        await simpleChatbot.initialize();
        setChatbot(simpleChatbot);
      } catch (error) {
        console.warn('Failed to initialize chatbot, using basic fallback:', error.message);
        const fallbackChatbot = new SimpleEducationalChatbot(null);
        await fallbackChatbot.initialize();
        setChatbot(fallbackChatbot);
      }
    };
  ...
  }, [isOpen, user, messages.length, chatbot]);

  const handleSendMessage = async (messageText = input) => {
    if (!messageText.trim() || !chatbot) return;
