-- ============================================
-- MIGRATION 010 - Système de points pour l'apprentissage
-- ============================================
-- Date: 6 octobre 2025
-- Description: Attribution automatique de points lors de la complétion de leçons

-- ============================================
-- Fonction : Calculer et attribuer les points pour une leçon complétée
-- ============================================
...
BEGIN
  ...
  FROM lecons l
  LEFT JOIN user_progress up ON up.lecon_id = l.id AND up.user_id = p_user_id
  ...
  IF v_completed_lessons_in_chapter = v_total_lessons_in_chapter THEN
    ...
    LEFT JOIN LATERAL (
      SELECT 
        ...
        COUNT(CASE WHEN up.completed = TRUE THEN 1 END) as completed,
        CASE 
          WHEN COUNT(*) = COUNT(CASE WHEN up.completed = TRUE THEN 1 END) 
          THEN TRUE 
          ELSE FALSE 
...
-- ============================================
-- Fonction : Obtenir les statistiques d'apprentissage
-- ============================================
...
BEGIN
  ...
  SELECT 
    ...
    (SELECT COUNT(*) FROM user_progress WHERE user_id = p_user_id AND completed = TRUE)::INT,
    ...
    (SELECT COUNT(DISTINCT c.id)
     ...
     WHERE NOT EXISTS (
       ...
       LEFT JOIN user_progress up ON up.lecon_id = l.id AND up.user_id = p_user_id
       WHERE l.chapitre_id = c.id AND (up.completed IS NULL OR up.completed = FALSE)
     ))::INT,
    ...
    (SELECT COUNT(DISTINCT m.id)
     ...
     WHERE NOT EXISTS (
       ...
       LEFT JOIN user_progress up ON up.lecon_id = l.id AND up.user_id = p_user_id
       WHERE c.matiere_id = m.id AND (up.completed IS NULL OR up.completed = FALSE)
     ))::INT,
...
SELECT 'Migration 010 : Fonctions de points pour l''apprentissage créées avec succès !' AS message;
